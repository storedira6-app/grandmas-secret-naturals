...
export function BottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 mx-auto max-w-md px-3 pb-3">
      <div className="glass-card flex items-center justify-around rounded-3xl px-1.5 py-2">
        {ITEMS.map(({ to, key, Icon }) => {
          ...
          return (
            <Link
              key={to}
              to={to}
              className="relative flex min-w-0 flex-1 flex-col items-center gap-1 rounded-2xl px-0.5 py-2 transition-all duration-300"
            >
              <span
